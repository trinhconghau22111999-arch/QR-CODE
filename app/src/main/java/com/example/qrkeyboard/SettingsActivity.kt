...
class SettingsActivity : AppCompatActivity() {
    ...
    // THEM (theo yeu cau nguoi dung: "thanh truot cho toc do chuyen mau
    // nhanh cham cua den vien phim"): tuong tu vibrationSeekBar/
    // vibrationValueText ben duoi - 1 SeekBar rieng cho toc do hieu ung RGB.
    ...
    private fun buildRgbEffectSection(): View {
        ...
        // chuyen mau nhanh cham cua den vien phim"): giong het cau truc
        // SeekBar cuong do rung ([buildVibrationSection]) - keo la luu +
        // ap dung NGAY (khong can khoi dong lai ban phim). Boc trong 1
        // LinearLayout rieng ([rgbSpeedSection]) de co the AN/HIEN theo
        // dung trang thai bat/tat hieu ung RGB (xem [refreshRgbEffectUi]) -
        // giu logic giong het rgbColorModeRow/rgbDirectionRow ben tren
        // (khong co y nghia gi khi hieu ung dang TAT).
        rgbSpeedSection = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }
        rgbSpeedSection.addView(sectionSubtitle("T\u1ed1c \u0111\u1ed9 chuy\u1ec3n m\u00e0u"))
        rgbSpeedSection.addView(spacer(6))
        val speedRow = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
        rgbSpeedValueText = TextView(this).apply {
            textSize = 14f
            setTextColor(textPrimary)
            layoutParams = LinearLayout.LayoutParams(dp(56), ViewGroup.LayoutParams.WRAP_CONTENT)
        }
        speedRow.addView(rgbSpeedValueText)
        rgbSpeedSeekBar = SeekBar(this).apply {
            max = 100
            progress = RgbEffectPrefs.getSpeedPercent(this@SettingsActivity)
            ...
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                    rgbSpeedValueText.text = "$progress%"
        ...
        rgbSpeedSection.addView(speedRow)
        rgbSpeedValueText.text = "${rgbSpeedSeekBar.progress}%"
        wrap.addView(rgbSpeedSection)
    ...
    private fun refreshRgbEffectUi() {
        ...
        rgbSpeedSection.visibility = if (enabled) View.VISIBLE else View.GONE
        if (!enabled) return
